...
const EditCheckInScreen = () => {
  ...
  // test location access code start
  ...
  const checkLocationServiceAndNavigate = async () => {
    ...
    try {
      ...
      if (result === RESULTS.GRANTED) {
        ...
        // Handle other statuses like RESULTS.UNAVAILABLE or RESULTS.LIMITED
        showPermissionModal(
          'Location Permission Required. Would you like to grant permission?',
          async () => {
            const requestResult = await request(permission);
  ...
  const fetchCurrentLocation = () => {
    Geolocation.getCurrentPosition(
      ...
      error => {
        ...
        setAlertModal({
          open: true,
          message: `Could not fetch current location. ${errorMessage}`,
          success: false,
        });
        setIsLoading(prev => ({...prev, loadMap: false})); // Stop loading indicator
      },
      ...
      [
        {
          text: 'Cancel',
          style: 'cancel',
          onPress: () => setIsLoading(prev => ({...prev, loadMap: false})),
        },
        {
          text: 'Open Settings',
          onPress: () => {
  ...
  const handleSubmit = async () => {
    ...
    //   formData.append('sauceId', _id); // Assuming item._id contains the sauceId
      ...
      // const response = await axios.post(`${host}/checkin`, formData, {
        const response = await axiosInstance.put(`/edit-checkin/${_id}`, formData, {
...
const handleFetchCheckin =useCallback(async ()=>{
   const res = await axiosInstance.get(`/get-checkin/${_id}`)
   setData({
    description: res.data.checkin?.text,
    address: res.data.checkin?.address,
    coordinates: {
      latitude: res.data.checkin?.latitude,
      longitude: res.data.checkin?.longitude,
    },
    foodPairings: res.data.checkin?.foodPairings,
   })
   setCurrentURLS(res.data.checkin.images||[])
},[])
            ...
            <View style={{alignItems: 'center', gap: 20}}>
              ...
              <CustomButtom
                ...
                buttonstyle={{
                  ...
                  alignItems: 'center',
                  justifyContent: isloading?.loadMap
                    ? 'center'
                    : 'space-between',
                }}
